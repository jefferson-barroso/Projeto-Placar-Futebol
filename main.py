...
while True:
  ...
  for i in range(0, partida):
    gol.append(int(input(f'Quantos gols jogador {jogador["nome"]} marcou na   partida {i}: ')))
...
print('-=' *35)
print(f'{"N°":<5}{"Nome":<20}{"Gols"}')
for i, v in enumerate(lista):
  print(f'{i:<3} jogador {v["nome"]:<10}{v["gols"]}')
print('-='*35)
id=0
while id != 999:
  id=int(input('Mostrar dados de qual jogador? (999 preparar): '))
  for i, v in enumerate(lista):
      if i == id:
